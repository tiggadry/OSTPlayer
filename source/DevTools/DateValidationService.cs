    ...
    public static class DateValidationService
    {
        ...
        public static DateValidationResult ValidateDate(string dateString, string context = "")
        {
            ...
            if (IsBlacklistedDate(dateString))
            {
                ...
                {
                    IsValid = false,
                    ErrorMessage = $"Date '{dateString}' is blacklisted - commonly found in documentation but incorrect for new updates",
                    SuggestedDate = DateHelper.GetCurrentDateString(),
                    RecommendedAction = DateValidationAction.UseSystemDate,
                ...
                System.Globalization.DateTimeStyles.None, out DateTime parsedDate))
            {
                ...
                {
                    IsValid = false,
                    ErrorMessage = $"Invalid date format: '{dateString}'. Expected YYYY-MM-DD format",
                    SuggestedDate = DateHelper.GetCurrentDateString(),
                    RecommendedAction = DateValidationAction.UseSystemDate,
            ...
            if (daysDifference > MaxFutureDays)
            {
                ...
                {
                    IsValid = false,
                    ErrorMessage = $"Date '{dateString}' is too far in the future ({daysDifference:F0} days from now)",
                    SuggestedDate = DateHelper.GetCurrentDateString(),
                    RecommendedAction = DateValidationAction.UseSystemDate,
            ...
            if (daysDifference < -MaxPastDays)
            {
                ...
                {
                    IsValid = false,
                    ErrorMessage = $"Date '{dateString}' is too far in the past ({Math.Abs(daysDifference):F0} days ago) for new content",
                    SuggestedDate = DateHelper.GetCurrentDateString(),
        ...
        public static List<DateValidationResult> ValidateMultipleDates(Dictionary<string, string> dates)
        {
        ...
        public static List<ProblematicDateFound> ScanFileForProblematicDates(string filePath)
        {
            ...
            try
            {
                ...
                foreach (Match match in matches)
                {
                    ...
                    {
                        problems.Add(new ProblematicDateFound
                        {
                            FilePath = filePath,
        ...
        public static ProjectDateValidationReport ScanProject(string projectPath = ".")
        {
            ...
            try
            {
                ...
                report.FilesWithProblems = report.ProblematicFiles.Select(p => p.FilePath).Distinct().Count();
                report.BlacklistedDatesFound = report.ProblematicFiles
                    .Count(p => p.ValidationResult.ValidationFlags.HasFlag(DateValidationFlags.BlacklistedDate));
            }
            catch (Exception ex)
            {
        ...
        public static DateValidationResult ValidateForAI(string proposedDate, AIOperationType operationType)
        {
