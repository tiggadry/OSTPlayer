...
namespace OstPlayer.DevTools {
    ...
    public static class DevToolsConfig {
        ...
        private static string SerializeConfiguration(DevToolsConfiguration config) {
            ...
            json.AppendLine("{");
            json.AppendLine($"  \"AutoUpdateDocumentation\": {config.AutoUpdateDocumentation.ToString().ToLower()},");
            json.AppendLine($"  \"AutoGenerateFileHeaders\": {config.AutoGenerateFileHeaders.ToString().ToLower()},");
            json.AppendLine($"  \"AutoUpdateFileHeaders\": {config.AutoUpdateFileHeaders.ToString().ToLower()},");
            json.AppendLine($"  \"EnableAIHooks\": {config.EnableAIHooks.ToString().ToLower()},");
            json.AppendLine($"  \"EnableTemplateEngine\": {config.EnableTemplateEngine.ToString().ToLower()},");
