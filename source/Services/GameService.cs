...
//
// FUTURE REFACTORING:
// TODO: Add persistent cache storage for game metadata
// TODO: Implement smart cache invalidation based on filesystem changes
// TODO: Add support for network-based music libraries
// TODO: Implement music library indexing for faster searches
// TODO: Add support for custom music file naming conventions
...
namespace OstPlayer.Services
{
    ...
    public class GameService : IGameService, IDisposable
    {
            ...
            ErrorHandlingService errorHandler = null)
        {
            ...
            this.playniteApi = playniteApi ?? throw new ArgumentNullException(nameof(playniteApi), "Playnite API is required");
        ...
        #region IGameService Implementation - Game Discovery and Access
        
        /// <summary>
        /// Gets all games that contain music files.
        /// </summary>
        public async Task<List<Game>> GetGamesWithMusicAsync(CancellationToken cancellationToken = default)
        {
            if (disposed)
            ...
            try
            {
                ...
                if (cachedGamesWithMusic != null && 
                    DateTime.Now - lastGameScanTime < cacheValidityPeriod)
                {
                    logger.Debug($"Returning cached games with music ({cachedGamesWithMusic.Count} games)");
                ...
                return await Task.Run(() =>
                {
                    ...
                    var gamesWithMusic = playniteApi.Database.Games.Where(game =>
                    {
                        cancellationToken.ThrowIfCancellationRequested();
                        ...
                        {
                            logger.Warn(ex, $"Error checking music files for game: {game.Name}");
                            return false;
                        }
                    })
                    .OrderBy(g => g.Name)
        ...
        /// </summary>
        public async Task<GameMusicInfo> GetGameMusicInfoAsync(Game game, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
        ...
        /// </summary>
        public async Task<List<Game>> FilterGamesAsync(string searchText, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
            ...
            try
            {
                ...
                return await Task.Run(() =>
                {
                    ...
                    return allGames.Where(game =>
                        ...
                        (game.Description?.ToLowerInvariant().Contains(searchLower) ?? false) ||
                        (game.Developers?.Any(d => d.Name.ToLowerInvariant().Contains(searchLower)) ?? false) ||
                        (game.Publishers?.Any(p => p.Name.ToLowerInvariant().Contains(searchLower)) ?? false)
                    ).ToList();
                }, cancellationToken);
            }
            catch (OperationCanceledException)
        ...
        /// </summary>
        public async Task<Game> FindGameByNameAsync(string gameName, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
        ...
        #region IGameService Implementation - Music File Operations
        
        /// <summary>
        /// Gets music files for a specific game.
        /// </summary>
        public async Task<List<string>> GetMusicFilesAsync(Game game, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
        ...
        /// </summary>
        public async Task<string> GetMusicDirectoryAsync(Game game, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
        ...
        /// </summary>
        public async Task<bool> ValidateMusicFileAsync(string filePath, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
        ...
        /// </summary>
        public async Task<List<string>> ScanForMusicFilesAsync(Game game, IProgress<int> progress = null, 
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                return await Task.Run(() =>
                {
                    ...
                    var allFiles = Directory.GetFiles(musicDirectory, "*.*", SearchOption.AllDirectories)
        ...
        #region IGameService Implementation - Batch Operations
        
        /// <summary>
        /// Scans all games for music files with progress reporting.
        /// </summary>
        public async Task<Dictionary<Game, List<string>>> ScanAllGamesForMusicAsync(IProgress<ScanProgress> progress = null,
            CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
            ...
            try
            {
                ...
                foreach (var game in allGames)
                {
                    ...
                    try
                    {
                        var musicFiles = await ScanForMusicFilesAsync(game, null, cancellationToken);
                ...
                logger.Info($"Batch scan completed. Found music in {results.Count} games ({totalFoundFiles} total files)");
                return results;
            }
            catch (OperationCanceledException)
            {
        ...
        /// </summary>
        public async Task<Dictionary<Game, List<string>>> ValidateAllMusicFilesAsync(IProgress<ValidationProgress> progress = null,
            CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
            ...
            try
            {
                ...
                foreach (var gameFile in allMusicFiles)
                {
                    ...
                    try
                    {
                        var isValid = await ValidateMusicFileAsync(gameFile.FilePath, cancellationToken);
                        if (isValid)
                        {
                ...
                logger.Info($"Batch validation completed. Valid: {validFiles}, Invalid: {invalidFiles}");
                return results;
            }
            catch (OperationCanceledException)
            {
        ...
        /// </summary>
        public async Task<MusicLibraryStatistics> GetLibraryStatisticsAsync(CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
            ...
            try
            {
                ...
                return await Task.Run(async () =>
                {
                    ...
                    var statistics = new MusicLibraryStatistics
                    {
                        LastScanned = DateTime.Now
                    };
                    ...
                    logger.Debug($"Library statistics calculated: {totalFiles} files, {totalSize} bytes");
                    return statistics;
                }, cancellationToken);
            }
            catch (OperationCanceledException)
        ...
        #region IGameService Implementation - Game Metadata
        
        /// <summary>
        /// Gets or creates music metadata directory for game.
        /// </summary>
        public async Task<string> EnsureMusicMetadataDirectoryAsync(Game game, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
        ...
        /// </summary>
        public async Task<bool> HasCachedMetadataAsync(Game game, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
        ...
        /// </summary>
        public async Task CleanupOrphanedMetadataAsync(CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(GameService));
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    var allGameIds = playniteApi.Database.Games.Select(g => g.Id.ToString()).ToHashSet();
                    ...
                    foreach (var orphanedDir in orphanedDirs)
                    {
                        ...
                        {
                            logger.Warn(ex, $"Failed to remove orphaned directory: {orphanedDir}");
                        }
                    }
                    
                    logger.Info($"Cleanup completed. Removed {orphanedDirs.Count} orphaned directories");
                }, cancellationToken);
            }
            catch (OperationCanceledException)
            {
        ...
        #region IGameService Implementation - Configuration and Health
        ...
        public async Task<ServiceHealthStatus> CheckHealthAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    healthStatus.Details["FileSystemAccess"] = canAccess ? "Available" : "Unavailable";
