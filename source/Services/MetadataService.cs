...
//
// FUTURE REFACTORING:
// TODO: Implement persistent cache storage with SQLite
// TODO: Add metadata source prioritization and weighting
// TODO: Implement real-time metadata updates and notifications
// TODO: Add metadata conflict resolution UI
// TODO: Extract caching to separate configurable service
// TODO: Add metadata export/import functionality
// TODO: Implement metadata quality scoring algorithms
// TODO: Add batch processing for multiple files
// TODO: Implement metadata source health monitoring
// TODO: Add user feedback integration for metadata quality
    ...
    public class MetadataService : IMetadataService, IDisposable
    {
            ...
            MetadataCacheConfig customCacheConfig = null)
        {
            ...
            try
            {
                ...
                cacheConfig = customCacheConfig ?? new MetadataCacheConfig
                {
                ...
                this.logger.Info($"MetadataService initialized successfully with DI (Max: {cacheConfig.MaxCacheSize} items)");
            }
            catch (Exception ex)
            {
                this.logger.Error(ex, "Failed to initialize MetadataService with dependency injection");
        ...
        /// </summary>
        public async Task<TrackMetadataModel> LoadTrackMetadataAsync(string filePath, CancellationToken cancellationToken = default)
        {
            if (disposed)
            ...
            try
            {
                ...
                var discogsMetadata = await LoadDiscogsMetadataAsync(mp3Metadata, cancellationToken);
                var musicBrainzMetadata = await LoadMusicBrainzMetadataAsync(mp3Metadata, cancellationToken);
                
                // Merge metadata from all sources
                var mergedMetadata = MergeMetadataFromSources(mp3Metadata, discogsMetadata, musicBrainzMetadata);
        ...
        /// </summary>
        public async Task<List<TrackMetadataModel>> LoadMultipleTracksMetadataAsync(IEnumerable<string> filePaths, 
            IProgress<int> progress = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                logger.Info($"Successfully loaded metadata for {results.Count}/{filePathList.Count} tracks");
                return results;
            }
            catch (OperationCanceledException)
            {
                logger.Info($"Multiple tracks metadata loading cancelled after {processedCount}/{filePathList.Count} files");
                throw;
            }
            catch (Exception ex)
            {
                logger.Error(ex, $"Error loading multiple tracks metadata after {processedCount}/{filePathList.Count} files");
        ...
        /// </summary>
        public async Task SaveTrackMetadataAsync(string filePath, TrackMetadataModel metadata, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(MetadataService));
        ...
        /// </summary>
        public async Task<AlbumMetadataModel> LoadAlbumMetadataAsync(Game game, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(MetadataService));
            ...
            try
            {
                ...
                logger.Debug($"Album TTL cache miss for game: {game.Name}, loading from sources...");
                
                // Load album metadata from external sources
                var albumMetadata = await LoadAlbumFromExternalSources(game.Name, game.Name, cancellationToken);
        ...
        /// </summary>
        public async Task<DiscogsMetadataModel> LoadDiscogsMetadataAsync(Game game, string discogsToken, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(MetadataService));
            ...
            try
            {
                ...
                var cachedDiscogs = metadataCache.GetExternalMetadata<DiscogsMetadataModel>(discogsKey);
                ...
                var discogsResults = await discogsClient.SearchReleaseAsync(game.Name, discogsToken, cancellationToken);
        ...
        /// </summary>
        public async Task<MusicBrainzMetadataModel> LoadMusicBrainzMetadataAsync(Game game, CancellationToken cancellationToken = default)
        {
            if (disposed)
                throw new ObjectDisposedException(nameof(MetadataService));
            ...
            try
            {
                ...
                var mbMetadata = await musicBrainzClient.SearchReleaseAsync(game.Name, game.Name, cancellationToken);
        ...
        public void Configure(OstPlayerSettings settings)
        {
            ...
            try
            {
                ...
                if (settings.EnableMetadataCache)
                {
                    ...
                    logger.Info($"Metadata cache enabled with TTL: {settings.MetadataCacheTTLHours} hours, Max size: {settings.MaxCacheSize}");
                }
        ...
        public async Task<ServiceHealthStatus> CheckHealthAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    healthStatus.Details["MusicBrainzAPI"] = mbHealthy ? "Available" : "Unavailable";
                }
                catch (Exception ex)
                {
        ...
        private async Task<TrackMetadataModel> LoadMp3MetadataAsync(string filePath, CancellationToken cancellationToken)
        {
            try
            {
                return await Task.Run(() =>
        ...
        private async Task<DiscogsMetadataModel> LoadDiscogsMetadataAsync(TrackMetadataModel mp3Metadata, CancellationToken cancellationToken)
        {
            if (mp3Metadata == null) return null;
            ...
            try
            {
                ...
                var results = await discogsClient.SearchReleaseAsync($"{mp3Metadata.Artist} {mp3Metadata.Album}", string.Empty);
        ...
        private async Task<MusicBrainzMetadataModel> LoadMusicBrainzMetadataAsync(TrackMetadataModel mp3Metadata, CancellationToken cancellationToken)
        {
            if (mp3Metadata == null) return null;
            ...
            try
            {
                ...
                var mbMetadata = await musicBrainzClient.SearchReleaseAsync(mp3Metadata.Artist, mp3Metadata.Album, cancellationToken);
        ...
        private async Task<AlbumMetadataModel> LoadAlbumFromExternalSources(string artist, string album, CancellationToken cancellationToken)
        {
            try
            {
                cancellationToken.ThrowIfCancellationRequested();
                
                // TODO: Implement album-specific external source loading
            ...
            {
                logger.Error(ex, $"Failed to load album metadata from external sources: {artist} - {album}");
                return null;
            }
        }
            ...
            MusicBrainzMetadataModel musicBrainzMetadata)
        {
            try
            {
                ...
                {
                    if (string.IsNullOrEmpty(mergedMetadata.Album) && !string.IsNullOrEmpty(musicBrainzMetadata.Album))
                        mergedMetadata.Album = musicBrainzMetadata.Album;
                        
                    if (string.IsNullOrEmpty(mergedMetadata.Artist) && !string.IsNullOrEmpty(musicBrainzMetadata.Artist))
                ...
                if (discogsMetadata != null)
                {
                    if (string.IsNullOrEmpty(mergedMetadata.Album) && !string.IsNullOrEmpty(discogsMetadata.Album))
                        mergedMetadata.Album = discogsMetadata.Album;
                        
                    if (string.IsNullOrEmpty(mergedMetadata.Artist) && !string.IsNullOrEmpty(discogsMetadata.Artist))
